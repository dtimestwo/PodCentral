-- Migration: Add transaction support for podcast sync
-- This adds unique constraints and RPC functions for atomic sync operations

-- ============================================
-- UNIQUE CONSTRAINTS
-- ============================================

-- Add unique constraint for podcast_funding
ALTER TABLE podcast_funding
  ADD CONSTRAINT podcast_funding_podcast_id_unique UNIQUE (podcast_id);

-- Add unique partial index for value_configs (podcast-level only)
CREATE UNIQUE INDEX value_configs_podcast_unique
  ON value_configs (podcast_id)
  WHERE episode_id IS NULL;

-- Add unique constraint for persons
ALTER TABLE persons
  ADD CONSTRAINT persons_name_role_unique UNIQUE (name, role);
...
BEGIN
  ...
  -- Insert new chapters if provided
...
BEGIN
  ...
  IF p_persons IS NULL OR jsonb_array_length(p_persons) = 0 THEN
    RETURN 0;
  END IF;
...
-- ============================================
-- MAIN SYNC RPC FUNCTION
-- ============================================

-- Main atomic sync function for podcast data
...
BEGIN
  ...
  IF p_podcast->'value' IS NOT NULL AND p_podcast->'value'->'model' IS NOT NULL THEN
    ...
    IF p_podcast->'value'->'destinations' IS NOT NULL THEN
  ...
  IF p_episodes IS NOT NULL THEN
    ...
    LOOP
      ...
      -- Sync persons for this episode (only for new episodes to match original behavior)
      IF v_existing_id IS NULL AND v_episode->'persons' IS NOT NULL THEN
        PERFORM sync_episode_persons(v_episode_id, v_episode->'persons');
      END IF;

      -- Sync soundbites (only for new episodes)
      IF v_existing_id IS NULL THEN
        -- Handle soundbites array
        IF v_episode->'soundbites' IS NOT NULL THEN
        ...
        ELSIF v_episode->'soundbite' IS NOT NULL THEN
          INSERT INTO soundbites (episode_id, start_time, duration, title)
          VALUES (
            v_episode_id,
        ...
        -- Sync social interact
        IF v_episode->'social_interact' IS NOT NULL THEN
